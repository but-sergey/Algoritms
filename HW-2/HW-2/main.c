...
int Calc1(int min, int max)
{
	...
	int num;
	// сумма всех комманд (для контроля окончания перебора)ж
	int sum;
	...
	do
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				result++;
				// изменение последней команды (для исключения повторений)
				commands[i] += 1;
				for (int j = i; j > 0; j--)
				{
					if (commands[i] > 2)
	...
	// выход из цикла - если все команды = 2 (сумма значений комманд = количество команд * 2)
	} while (sum < count * 2);
